...
export function InputField({ icons, type, name, placeholder, isError, onIconClick, inputValue, onChange }: InputFieldProps) {
    ...
    return (
        <div className={`w-full flex flex-row items-center justify-between gap-x-2 px-4 py-3 rounded-8 border border-neutral-300 cursor-pointer
                hover:bg-neutral-50
                ${inputValue && inputValue.length > 0 ? "ring ring-neutral-950" : ""}
                ${isError ? "border-neutral-red-500 ring-0" : ""}
            `}>
            <div className="w-full flex flex-row gap-x-2">
                ...
                <input type={type} name={name} id={name} placeholder={placeholder}
                    ...
                    className="w-full sans-serif-text-preset-5 text-neutral-950
